...
export const useCartItems = () => {
    ...
    const [addProductToCartMutation, { data: newProductData, loading: newProductLoading }] =
        useAddProductToCartMutation({ fetchPolicy: "no-cache" });
    ...
    const updateProduct = async (cartId: string, cartItemId: string, quantity: number) => {
        setPermission(false);
    ...
    const addItems = async (product: CartItem) => {
        ...
        const incrementProductQuantity = await updateProduct(
            data.cart.id,
            existingItem.id,
